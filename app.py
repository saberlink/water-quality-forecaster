...
def calculate_wqi_precise(row, use_case="Drinking Water"):
    ...
    for param, data in available_params.items():
        ...
        if stats['type'] == 'normal':
            qn = 100 * ((vn - stats['Vid']) / (stats['Sn'] - stats['Vid']))
        ...
        elif stats['type'] == 'inverse':
            qn = 100 * ((stats['Vid'] - vn) / (stats['Vid'] - stats['Sn']))
...
if 'clean_history' not in st.session_state:
    st.session_state.clean_history = None
...
if 'model_assets' not in st.session_state:
    st.session_state.model_assets = None
    ...
    if st.session_state.clean_history is not None:
        with st.spinner('Forecasting...'):
            ...
            try:
                ...
                if len(f_cols) > 17:
                    f_cols = f_cols[:17]  # Match App 9 safety clip
                ...
                with c1:
                    ...
                    fig_t.add_trace(go.Scatter(
                        x=baseline['Timestamp'], y=baseline['Turbidity'],
                        mode='lines', name='Turbidity',
                ...
                with c2:
                    ...
                    fig_e.add_trace(go.Scatter(
                        x=baseline['Timestamp'], y=baseline['EC'],
                        mode='lines', name='EC',
...
# ==========================================
# STEP 3: EXPLAIN (SHAP XAI)  ✅ MODIFIED PART ONLY
# ==========================================
st.divider()
...
if st.button("Run SHAP Analysis"):
    if st.session_state.model_assets is None:
        ...
        with st.spinner("Calculating Feature Importance..."):
            try:
                # ---- Load assets exactly as trained in Step 2 ----
                model, scaler_X, _, f_cols = st.session_state.model_assets
                df = st.session_state.clean_history
                eng_df = engineer_features(df.copy())

                # ---- Safety: ensure enough rows for sequences ----
                if len(eng_df) <= SEQUENCE_LENGTH:
                    st.error(f"Not enough data ({len(eng_df)} rows) to build sequences of length {SEQUENCE_LENGTH}.")
                    st.stop()

                # ---- Build 2D flattened sequence matrix (same as before) ----
                X = eng_df[f_cols].values
                X_scaled = scaler_X.transform(X)

                # IMPORTANT FIX:
                # Your original range was (len(X_scaled) - SEQUENCE_LENGTH) which produces (n - L) sequences,
                # but the correct number of full windows is (n - L + 1).
                # This single off-by-one can change the last samples used and therefore SHAP outcomes.
                X_sequences = np.array([
                    X_scaled[i:i + SEQUENCE_LENGTH]
                    for i in range(len(X_scaled) - SEQUENCE_LENGTH + 1)
                ])

                X_2d = X_sequences.reshape(X_sequences.shape[0], -1)

                # ---- Predict wrapper for KernelExplainer ----
                def predict_fn_2d(X_flat):
                    X_3d = X_flat.reshape(X_flat.shape[0], SEQUENCE_LENGTH, len(f_cols))
                    return model.predict(X_3d, verbose=0)

                # ---- Optional: make SHAP repeatable ----
                # np.random.seed(0)

                # ---- Background sampling (same logic, but safe) ----
                n_bg = min(50, X_2d.shape[0])
                bg_idx = np.random.choice(X_2d.shape[0], n_bg, replace=False)
                background = X_2d[bg_idx]

                explainer = shap.KernelExplainer(predict_fn_2d, background)

                # Explain last 5 samples (same intent, but safe if fewer)
                n_explain = min(5, X_2d.shape[0])
                shap_values = explainer.shap_values(X_2d[-n_explain:])

                # ---- Plot importance per target (same aggregation) ----
                c1, c2 = st.columns(2)
                ...
                for i, target in enumerate(targets):
                    ...
                    vals = np.mean(
                        np.abs(shap_values[i].reshape(-1, SEQUENCE_LENGTH, len(f_cols))),
                        axis=(0, 1)
                    )
